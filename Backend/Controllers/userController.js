      ...
      .populate({
        ...
        populate: [
          ...
          },
          {
            path: "dishes.dish",
            model: "Dish",
            select: "name price", // Add any other fields you want to include
          },
        ],
      });
...
exports.removeSavedRecipe = async (req, res) => {
  try {
    ...
    console.log(
      `Attempting to remove recipe ${recipeId} and for user ${userId}`
    );

    console.log(
      "Before update:",
      await User.findById(userId).select("savedRecipes")
    );
    ...
    console.log("After update:", user.savedRecipes);
    ...
    console.log(`Updated savedRecipes for user ${userId}:`, user.savedRecipes);
